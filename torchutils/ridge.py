...
class MultiRidge:
    ...
    def fit(self, X, Y):
        ...
        self.ps = []
        self.rs = []
        for Y_j, Ym_j in zip(Y.t(), self.Ym):
            p_j = X_t @ (Y_j - Ym_j)
            self.ps.append(p_j)
            self.rs.append(self.Q.t() @ p_j)
        return self

    def _predict_single(self, X_te_p, M_te, y_idx, l, N_te_j=None):
        Ym_j, p_j, r_j = self.Ym[y_idx], self.ps[y_idx], self.rs[y_idx]
        if N_te_j is None:
            N_te_j = X_te_p @ p_j
        Yhat_te_j = (1 / l) * (N_te_j - M_te @ (r_j / (self.e + l))) + Ym_j
        return Yhat_te_j, N_te_j
    ...
    def predict_single(self, X_te, l_idxs):
        ...
        for j, l_idx in enumerate(l_idxs):
            l = self.ls[l_idx]
            Yhat_te_j, _ = self._predict_single(X_te, M_te, j, l)
...
class RidgeCVEstimator:
    def __init__(self, ls, cv, scoring, scale_X=True, scale_thresh=1e-8):
        ...
        """
        _validate_ls(ls)
        self.base_ridge = MultiRidge(ls, scale_X, scale_thresh)
        self.cv = cv
